...
class Mongo extends Driver
{
    ...
    /**
     * @var $_dbName string dbName
     */
    protected $_dbName = '';
    /**
     * @var $_collectionName string collectionName
     */
    protected $_collectionName = '';
    /**
     * @var $_cursor \MongoCursor
     */
    ...
    public function select(array $options = [])
    {
        ...
        $this->queryTimes++;
        $query = $this->parseWhere(isset($options['where']) ? $options['where'] : []);
        $field = $this->parseField(isset($options['field']) ? $options['field'] : []);
        try {
            if ($this->config['debug']) {
    ...
    {
        if (!empty($tableName) && $tableName != $this->_collectionName) {
            $this->switchCollection($tableName, '', false);
        }
        $this->queryTimes++;
        if ($this->config['debug']) {
    ...
    public function parseWhere($where)
    {
        ...
        if (isset($where['_logic'])) {
            ...
            $_logic = in_array($where['_logic'], ['or', 'xor', 'nor', 'and']) ? '$'.$where['_logic'] : $_logic;
            unset($where['_logic']);
        }
