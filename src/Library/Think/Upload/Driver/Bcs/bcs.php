...
//sdk superfile分片大小 ，单位 B（字节）
    ...
    class BaiduBCS
    {
         ...
         * @return BCS_ResponseCore
         */
        public function list_bucket($opt = [])
        {
         ...
         * @return BCS_ResponseCore
         */
        public function create_bucket($bucket, $acl = null, $opt = [])
        {
            $this->assertParameterArray($opt);
         ...
         * @return BCS_ResponseCore
         */
        public function get_bucket_acl($bucket, $opt = [])
        {
            $this->assertParameterArray($opt);
            $opt [self::BUCKET] = $bucket;
            $opt [self::METHOD] = 'GET';
         ...
         * @return BCS_ResponseCore
         */
        public function list_object($bucket, $opt = [])
        {
            $this->assertParameterArray($opt);
            $opt [self::BUCKET] = $bucket;
         ...
         * @return BCS_ResponseCore
         */
        public function list_object_by_dir($bucket, $dir = '/', $list_model = 2, $opt = [])
        {
            $this->assertParameterArray($opt);
            $opt [self::BUCKET] = $bucket;
            if (empty ($opt [self::BUCKET])) {
         ...
         * @return BCS_ResponseCore
         */
        public function create_object($bucket, $object, $file, $opt = [])
        {
            $this->assertParameterArray($opt);
            $opt [self::BUCKET] = $bucket;
         ...
         * @return BCS_ResponseCore
         */
        public function create_object_by_content($bucket, $object, $content, $opt = [])
        {
            $this->assertParameterArray($opt);
            $opt [self::BUCKET] = $bucket;
            $opt [self::OBJECT] = $object;
         ...
         * @return BCS_ResponseCore
         */
        public function create_object_superfile($bucket, $object, $file, $opt = [])
        {
            ...
            for ($i = 0; $i < $sliceNum; $i++) {
                ...
                $object_list ['object_list'] ['part_' . $i] = [];
         ...
         *                       1. 函数参数列表顺序需为
         *                       ($bucket,$object,$file,&$opt,$response)，注意$opt为upload_directory函数传入的$opt的拷贝，只对当前object生效
         *                       2. 函数返回值无要求
         *                       string seek_object 用户断点续传，需要为object名称，如果该object在目录中不存在，抛出异常，若存在则将该object和此后的object进行上传
         *                       string seek_object_id
         *                       作用同seek_object，只需要传入上传过程中日志中展示的[a/b]中object序号即可，注意object序号是以1开始计算的
         ...
         *
         */
        public function upload_directory($bucket, $dir, $opt = [])
        {
            $this->assertParameterArray($opt);
            if (!is_dir($dir)) {
         ...
         * @return BCS_ResponseCore
         */
        public function copy_object($source, $dest, $opt = [])
        {
            $this->assertParameterArray($opt);
         ...
         * @return BCS_ResponseCore
         */
        public function set_object_meta($bucket, $object, $meta, $opt = [])
        {
            $this->assertParameterArray($opt);
         ...
         * @return BCS_ResponseCore
         */
        public function get_object_acl($bucket, $object, $opt = [])
        {
            $this->assertParameterArray($opt);
            $opt [self::BUCKET] = $bucket;
            $opt [self::METHOD] = 'GET';
         ...
         * @return BCS_ResponseCore
         */
        public function set_object_acl($bucket, $object, $acl, $opt = [])
        {
            $this->assertParameterArray($opt);
            //analyze acl
            $result = $this->analyze_user_acl($acl);
         ...
         * @return BCS_ResponseCore
         */
        public function delete_object($bucket, $object, $opt = [])
        {
            $this->assertParameterArray($opt);
            $opt [self::BUCKET] = $bucket;
            $opt [self::METHOD] = 'DELETE';
         ...
         */
        public function is_object_exist($bucket, $object, $opt = [])
        {
            $this->assertParameterArray($opt);
            $opt [self::BUCKET] = $bucket;
         ...
         */
        public function get_object_info($bucket, $object, $opt = [])
        {
            $this->assertParameterArray($opt);
            $opt [self::BUCKET] = $bucket;
            $opt [self::METHOD] = 'HEAD';
         ...
         * @return BCS_ResponseCore
         */
        public function get_object($bucket, $object, $opt = [])
        {
            $this->assertParameterArray($opt);
         ...
         * 生成签名链接
         */
        private function generate_user_url($method, $bucket, $object, $opt = [])
        {
            $opt [self::AK] = $this->ak;
            $opt [self::SK] = $this->sk;
            $opt [self::BUCKET] = $bucket;
