<?php
    ...
    final class Dispatcher
    {
        ...
        public static function dispatch()
        {
            ...
            define(
                '__MODULE__',
                (defined('BIND_MODULE') || ! C('MULTI_MODULE')) ? __APP__ : __APP__.'/'.($urlCase ? strtolower(
                        $moduleName
                    ) : $moduleName)
            );
            ...
            if ('' != $_SERVER['PATH_INFO'] && ( ! C('URL_ROUTER_ON') || ! Route::check())) {
                ...
                // 检查禁止访问的URL后缀
                if (C('URL_DENY_SUFFIX') && preg_match(
                ...
                $_SERVER['PATH_INFO'] = preg_replace(
                    C('URL_HTML_SUFFIX') ? '/\.('.trim(C('URL_HTML_SUFFIX'), '.').')$/i' : '/\.'.__EXT__.'$/i',
                    '',
                    $_SERVER['PATH_INFO']
                );
            ...
            define(
                '__CONTROLLER__',
                __MODULE__.$depr.(defined('BIND_CONTROLLER') ? '' : ($urlCase ? parse_name(
                    $controllerName
                ) : $controllerName))
            );
